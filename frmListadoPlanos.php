...
if($recarga == 2){
	...
	$sqlA = " SELECT A.idTematicaPadre, A.nombreTematica, B.numeroLote, B.nombreLote, C.numeroPlano, C.descripcionPlano, 
	D.*, D1.nombreOriginaMod, D2.nombreClaseMod
	FROM Tematicas A, LotesTrabajo B, Planos C, Revisiones D, TipoOriginaModificacion D1, TipoClaseModificacion D2
...
<table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#276074">
  <tr>
  ...
  <tr align="center">
	...
	<table width="60%" cellspacing="1">
      ...
      <tr>
        <td class="TituloTabla2">Tem&aacute;tica:</td>
        <td class="TxtTabla">
		<select name="lstTematica" class="CajaTexto" id="lstTematica" onChange="envia1()">
      ...
      <tr>
        ...
        <td class="TxtTabla">
		<select name="lstLote" class="CajaTexto" id="lstLote" onChange="envia1()">
      ...
      <tr>
        ...
        <td class="TxtTabla"><select name="lstRev" class="CajaTexto" id="lstRev">
            <option value="-1"> ::: Revisión Reciente ::: </option>
            <?
				for($i = 0; $i <= $maxRevisiones; $i++){
					$selRev = "";
					if($i == $lstRev && isset($lstRev)){
						$selRev = "selected";
					}
			?>
            <option value="<? echo $i ?>" <? echo $selRev; ?>> <? echo "R".$i ?> </option>
            <?
				}
			?>
          </select>
    ...
    <td><table width="100%" cellspacing="1">
	  ...
	  <tr class="TxtTabla">
		...
		if($regA[idTematicaPadre] != $regA[idTematica]){
			$path = "../PHSFiles/" . $_SESSION["phsProyecto"] . "/T".$regA[idTematicaPadre]."/T".$regA[idTematica]."/".$regA[idLote]."/".$regA[numeroPlano]."/R".$regA[numeroRevision];
		}
		else{
			$path = "../PHSFiles/" . $_SESSION["phsProyecto"] . "/T".$regA[idTematica]."/".$regA[idLote]."/".$regA[numeroPlano]."/R".$regA[numeroRevision];
		}
		?>
        <td align="center">
        ...
        <td align="center"><? echo $regA[numeroRevision]; ?></td>
        <td>
		<?
        ...
        <td><? echo $regA[nombreClaseMod]; ?></td>
